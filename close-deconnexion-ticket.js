...
async function getDataFromHelpdesk() {
    ...
    const subjects =
        "'Déconnexion avec perte de synchro', 'Déconnexion sans perte de synchro', 'Déconnexion avec perte de synchro_cable sous terrain'";
...
async function filterTicketToClose(lastAuths, lastStops, ticketList) {
    ...
    for (const ticket of ticketList) {
        ...
        const DAY_WITH_MILLISECONDS_SECONDS = 86400000;
        if (
            stopInstanceStart &&
    ...
    return { ticketToClose, ticketWithoutTelAdsl};
}
...
async function createExcelFile(ticketToClose, gsmList) {
    ...
    await workbookGsm.csv.writeFile("gsm-deconnexion.csv");
    await workbook.xlsx.writeFile("close-deconnexion-ticket.xlsx");
}
...
async function deconnexionTicketClose() {
    console.log("-------------------- START CRON -----------------------");
    console.time("startCron");
    let fileExists = fs.existsSync("close-deconnexion-ticket.xlsx");
    if (fileExists) {
        fs.unlinkSync("close-deconnexion-ticket.xlsx");
        console.log("excel file removed");
    }

    let gsmExists = fs.existsSync("gsm-deconnexion.csv");
    if (gsmExists) {
        fs.unlinkSync("gsm-deconnexion.csv");
...
module.exports = { deconnexionTicketClose };
